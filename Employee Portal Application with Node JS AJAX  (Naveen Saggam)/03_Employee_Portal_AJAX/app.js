...
tableBody.addEventListener('click', function (e) {
    ...
    if (targetElement.classList.contains('update')) {
        ...
        http.get(url, (err, employees) => {
            ...
            populateUpdateModel(selectedEmployee);
        });
    };
});

let populateUpdateModel = (selectedEmployee) => {
    ...
    $('#update-employee-modal').modal('show'); // To show the modal
};
...
updateEmployeeForm.addEventListener('submit', function (e) {
    ...
    e.preventDefault();
    $('#update-employee-modal').modal('hide'); // To hide the modal
    let employee = {
